...
monteHePlot <- function(sumDF, colorField = "Sample", ribbon=FALSE){
  ...
  if(ribbon==FALSE){
    p <- ggplot(volDF) +
      ...
      labs(x="Source rock volume (km3)",y="He generation rate (mol/year)", color="Source Rock") +
      theme_bw() +
      scale_y_log10(sec.axis = sec_axis(~ . * 0.0040026, name = "(kg/year)")) +
      scale_x_log10()
  } else {
    p <- ggplot(volDF) +
      geom_line(mapping=aes(x=vol_km,y=HeRateMean_molyr, color=!!sym(colorField))) +
      geom_ribbon(mapping=aes(x=vol_km,ymin=HeRateMin_molyr,ymax=HeRateMax_molyr, fill=!!sym(colorField)),alpha=0.1) +
      theme_bw() +
      labs(x="Source rock volume (km3)",y="He generation rate (mol/year)", color="Source Rock") +
      scale_x_log10() +
        scale_y_log10(sec.axis = sec_axis(~ . * 0.0040026, name = "(kg/year)"))
